...
def set_text_unfinished():
    return gr.update(
        visible=True, value="<h3>(Not Finished) Generating ···  The intermediate results will be shown.</h3>"
    )
...
class SpatialAttnProcessor2_0(torch.nn.Module):
    ...
    def __init__(
        self, hidden_size=None, cross_attention_dim=None, id_length=4, device="cuda", dtype=torch.float16
    ):
        super().__init__()
...
def process_generation(
    ...
    if _model_type == "original":
        ...
        raise NotImplementedError(
            "You should choice between original and Photomaker!", f"But you choice {_model_type}"
        )
        ##### ########################
    pipe.scheduler = DDIMScheduler.from_config(pipe.scheduler.config)
    ...
    prompts = [
        general_prompt + "," + prompt if "[NC]" not in prompt else prompt.replace("[NC]", "")
            ...
            with gr.Column(visible=True) as gen_prompt_vis:
                ...
                with gr.Accordion("(4) Tune the hyperparameters", open=True):
                    ...
                    id_length_ = gr.Slider(
                        label="Number of id images in total images", minimum=2, maximum=4, value=2, step=1
                    )
                    ...
                    comic_type = gr.Radio(
                        ["No typesetting (default)", "Four Pannel", "Classic Comic Style"],
                    ...
                    Ip_Adapter_Strength = gr.Slider(
                        label="Ip_Adapter_Strength", minimum=0, maximum=1, step=0.1, value=0.5, visible=False
                    )
