    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None):
        ...
        if attn_count == total_count:
            ...
            mask256, mask1024, mask4096 = cal_attn_mask(
                self.total_length, self.id_length, sa16, sa32, sa64, device=self.device, dtype=self.dtype
            )
...
def cal_attn_indice_xl_effcient_memory(
    total_length, id_length, sa32, sa64, height, width, device="cuda", dtype=torch.float16
):
    nums_1024 = (height // 32) * (width // 32)
    nums_4096 = (height // 16) * (width // 16)
